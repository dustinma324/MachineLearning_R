...
```{r, load_df_01_data, eval=TRUE, message=FALSE}
...
``` {r, run_chart}
df_01 %>% ggplot(mapping = aes(obs_id,x)) +
  geom_line() +
  geom_point()
```
There are Two points that stands out from the rest. They are approximately located at `obs_id` 45 and 260.
...
```
Based on the histogram, the majority of the data is centered around 0, giving this histogram a bell curve shape like a gaussian distribution.
...
fifthq = quantile(df_01$x,0.05)
fifthq
nintyfifthq = quantile(df_01$x,0.95)
nintyfifthq
...
The statistics caluclated is in line with both the run chart and the histogram. Standard deviation is around $\sigma = 5.158638$, and the 5th and 95th percent quantile are located at $-7.785702$ and $9.234694$, respectively.
...
df_01 %>% 
  ...
  ggplot(mapping = aes(obs_id,rolling_avg)) +
  geom_line()
```
The begining of the rolling average appears to be more variable, and finally stables out around `obs_id` of 50 as more samples are added in.
...
```{r, rolling_average_samples}
value = df_01 %>% mutate(rolling_avg = cummean(x))
value[5,]
value[45,]
```
...
It is a good idea to transform sigma before performing the Laplace Approximation due to the bounds violationo of the posterior. By Applying the transiformation on $\sigma$, the bounds are enforced.
...
#### SOLUTION
Keeping the prior on $\mu$ and $\sigma$ general, the joint-posterior is,
$$  
p(\mu,\varphi|\bf{x}) = \prod_{n=1}^{N} \Big\{p\big(x_n|\mu,g^{-1}(\varphi)\big)\Big\} p\big(\mu|\mu_{0},\tau_{0}\big) p\big(g^{-1}(\varphi\big)|
l,u) * \bigg| \frac{d}{d\varphi} \big(g^{-1}(\varphi)\big) \bigg|
$$
...
quantile2 <- 0.99
### bounds on mu
mu_lwr <- qnorm(quantile1,info_inform_N05$mu_0,info_inform_N05$tau_0,log=FALSE)
mu_upr <- qnorm(quantile2,info_inform_N05$mu_0,info_inform_N05$tau_0,log=FALSE)

### bounds on varphi
varphi_lwr <- log(qexp(quantile1,info_inform_N05$sigma_rate))
varphi_upr <- log(qexp(quantile2,info_inform_N05$sigma_rate))

### create the grid 
param_grid <- expand.grid(mu = seq(mu_lwr, mu_upr, (mu_upr-mu_lwr)/(201-1)),
                          varphi = seq(varphi_lwr, varphi_upr, (varphi_upr-varphi_lwr)/(201-1)),
                          KEEP.OUT.ATTRS = FALSE,
                          stringsAsFactors = FALSE) %>% 
  as.data.frame() %>% tbl_df()
```
...
It seems like anything above $\varphi \approx< 1.8$ is ruled out. The most probably $\mu$ seems to be around $\mu \approx -0.5$. The posterior unertainty on $\mu$ is right around the prior uncertainty.
...
**Complete the two code chunks below. The first code chunk requires you to complete the `purrr::map2_dbl()` call, which evaluates the log-posterior for all combinations of the parameters. Be careful that you use the correct list of required information. The `log_post_inform_N45` object corresponds to the 45 sample case, while the `log_post_info_N405` object corresponds to using all of the samples.**  
...
With increasing sample size, the log-posterior surface decreases, giving a probable value of $\mu \approx 0.75$, and $\varphi \approx 1.75$. The uncertainty parameter also decreases with increasing sample size.
...
The optimal parameter values and the hessian matrix are very similar with small/neglegible differences. The log-posterior surface gave me a good hint by having one maximal value, almost hill like shape.
...
set.seed(200121)
sample_size <-1e4
post_samples_inform_N05 <- generate_post_samples(laplace_res_inform_N05, N = sample_size)
post_samples_inform_N45 <- generate_post_samples(laplace_res_inform_N45, N = sample_size)
post_samples_inform_N405 <- generate_post_samples(laplace_res_inform_N405, N = sample_size)
```
...
```{r, create_post_sample_summary, eval=TRUE}
summary(post_samples_inform_N05)
summary(post_samples_inform_N45)
summary(post_samples_inform_N405)
```

The mean for $\mu$ increased as sample size increased, and the mean for $\sigma$
decreased from sample size of 5 to 45 but increased from 45 to 405.

```{r, historgram_05}
num_bin <- 50
# 5 Sample Data
post_samples_inform_N05 %>% ggplot(mapping = aes(mu)) + geom_histogram(bins = num_bin)
post_samples_inform_N05 %>% ggplot(mapping = aes(sigma)) + geom_histogram(bins = num_bin)

# 45 Sample Data
post_samples_inform_N45 %>% ggplot(mapping = aes(mu)) + geom_histogram(bins = num_bin)
post_samples_inform_N45 %>% ggplot(mapping = aes(sigma)) + geom_histogram(bins = num_bin)

# 405 Sample Data
post_samples_inform_N405 %>% ggplot(mapping = aes(mu)) + geom_histogram(bins = num_bin)
post_samples_inform_N405 %>% ggplot(mapping = aes(sigma)) + geom_histogram(bins = num_bin)
```
...
The posterior probability that $\mu$ is positive for each of the three observation sample sizes are,
```{r, positive_probability05, eval=TRUE}
les05 <- post_samples_inform_N05 %>% 
  filter(mu >= 0) %>% 
  mutate(nu = n()) %>% 
  summarize(unique(nu/sample_size))
les05
```

```{r, positive_probability45, eval=TRUE}
les45 <- post_samples_inform_N45 %>% 
  filter(mu >= 0) %>% 
  mutate(nu = n()) %>% 
  summarize(unique(nu/sample_size))
les45
...
```{r, positive_probability405, eval=TRUE}
les405 <- post_samples_inform_N405 %>% 
  filter(mu >= 0) %>% 
  mutate(nu = n()) %>% 
  summarize(unique(nu/sample_size))
les405
...
The 5th and 95th percential for the different samples sizes are,
```{r, quantile_value_all_three, eval=TRUE}
...
```{r, read_in_df_02_datasets, eval=TRUE, message=FALSE}
...
```{r} 
# Low noise scatter plot
df_02_low %>% ggplot(mapping = aes(x,y)) + geom_point()

# High noise scatter plot
df_02_high %>% ggplot(mapping = aes(x,y)) + geom_point()
```
The funcational form of the low noise data seems to be of a parabolic function.
...
#### SOLUTION
The 8th order polynomail has the highest R-squared value for both the low noise and high noise case.
...
The quadratic model has the lower cross-validation average RMSE on the test splits.