...
$$
The size of $\mathbf{A}$ is $N \times H$.
...
$$
The response vector, $\mathbf{f}$, is a $N \times 1$ column vector.
...
$$
\mathrm{SSE} = (\mathbf{y}-\mathbf{f})^{T}(\mathbf{y}-\mathbf{f})
$$
...
### design matrix
Xmat <- model.matrix(y ~ x, data = train_data)
...
optim_fit_2_b$par
```
The optimal parameter values are not the same for the two starting guesses. The results are not the same because neural network is multimodal unlike that of the lapalce approximation. 
...
$$
\frac{\delta R_n}{\delta \alpha_0} = \frac{\delta}{\delta \alpha_0}(y_n - f_n)^2 = -2(y_n - f_n)
$$
$$
\frac{\delta R_n}{\delta \alpha_k} = \frac{\delta}{\delta \alpha_k}(y_n - f_n)^2 = -2(y_n - f_n)h_{n,k}
$$
...
$$
\sum_{n=1}^{N} \left( \frac{\partial R_n}{\partial \alpha_k} \right) = \frac{\delta}{\delta \alpha_k}(y - f)^{T}(y - f) = -2(\mathbf{y} - \mathbf{f})\mathbf{h}_{:,k}
$$ 
...
$$
\sum_{n=1}^{N} \left( \frac{\partial R_n}{\partial \alpha_k} \right) = -2 \mathbf{\delta} \mathbf{H} 
$$ 
The dimensions of $\mathbf{\delta}$ is $N \times 1$. We are missing the paramter with respect to $\alpha_0$.
...
$$
\mathbf{g}_{\alpha} = \sum_{n=0}^{N} \left( \frac{\partial R_n}{\partial \alpha_k} \right) = \frac{\delta}{\delta \alpha_k}(y - f)^{T}(y - f) = -2(\mathbf{y} - \mathbf{f})\mathbf{h}_{:,k} = -2 \mathbf{\delta} \mathbf{H} 
$$
where the first column, $\mathbf{h}_{:,0}$ is a vectors of 1's.
...
$$
\frac{\delta R_n}{\delta \beta_{dk}} = \frac{\delta}{\delta \beta_{dk}} \Big((y_n-f_n)^2\Big)= -2(y_n-f_n)\alpha_{k}g_{n,k}^{\prime}x_{n,d} = -2\alpha_{k}g_{n,k}^{\prime}\delta_{n}x_{n,d}
$$
Replaceing $\alpha_{k}g_{n,k}^{\prime}\delta_{n} = \xi_{n,k}$,
$$
\frac{\delta R_n}{\delta \beta_{dk}} = -2\xi_{n,k}x_{n,d}
$$
...
```
The values of beta are zero's because we are optimizing it, therefore hitting a saddle point of the betas, thus the derivative of beta parameter at a saddel point should be equivalent to zero.
...
```
The backwards error propagation method's optimal parameter are very different for the $\beta$ values, but close to the $\alpha$ values.
...
```
The run time for the backwards error propagation method is significantly faster than finite difference.
...
Based on the values returned, a signle layer with 5 hidden unit performed the best.
...
A single layer with 14 hidden units performed the best.
...
Based on the "one-standard error rule" concept, layer1 with 8 units is consider "good enough".