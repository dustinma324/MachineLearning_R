...
x_factor_2 <- cut(x_dbl, breaks = c(0,5,11), inlcude.lowest = TRUE) ### set the breaks and 
...
my_iris <- iris %>% 
  mutate(sepal_length_bin = cut(Sepal.Length, breaks = hist_breaks, inlcude.lowest = TRUE))
```
...
The results from previous part's x-axis is different because we are giving `ggplot()` a range of values instead of the number of bin we are using in problem 1. In other words, we are grouping the values that lands in each range instead of having `geom_histogram()` find the min and the max and divide the data into sections.
...
Compared to the `count()` function, this method returns the total number of dataset in `sepal_length_bin`. `count()` distributes the number of values that lands within each range where as this method returns the `size()` of the dataset in MATLAB language. 
...
Our manual histogram or stem plot consists of 4 groups, where as the histogram generated by `geom_freqpoly()` have 6 groups. I did play around with the bin number, and when bin = 4, the count from `geom_freqpoly()` over shoots what our manual histogram generated.
...
```{r, solution_05b}
diamonds %>% 
  ggplot(mapping = aes(color,price)) +
  geom_boxplot()
```

The price of each color tends to increase as you move from color D to J. The percentile range also increases as you move from color D to J. More outliers exists in the colors D and E compared to the rest of the colors, also the whiskers also increased as you move from color D to J.
...
By adding filters of `cut == "Ideal"` and `clarity == "IF"`, we limit the amount of data used in the diamond variable. With the filters applied, we are able to get a better idea about the prices of the different colors as well as how the distribution looks like with the conditions applieed. First off, color D's price increaased by a significant amount while the rest decreased compared to that of problem 5b). It is also evident that the IQR increased for color D while the rest significantly decreased, meaning the probability of an ideal cut diamond with clarity "IF" for color D varies quite a bit and the rest has a "standard" pricing. We should also note that there are some outliers to all of the colors except for D. Therefore, the range of the price for color D isn't as defined as the other colors.
...
$$ 
\frac{\partial f(x_{1},x_{2})}{\partial x_{1}} = abx_{1}^{b-1}
$$
...
$$ 
\frac{\partial f(x_{1},x_{2})}{\partial x_{2}} = cdx_{2}^{d-1}
$$
...
$$ 
\frac{\partial^{2} f(x_{1},x_{2})}{\partial x_{1}^{2}} = ab(b-1)x_{1}^{b-2}
$$
...
$$ 
\frac{\partial^{2} f(x_{1},x_{2})}{\partial x_{1} \partial x_{2}} = 0
$$
...
$$ 
\langle\mathbf{x}_{\left(N \times 1\right)},\mathbf{x}_{\left(N \times 1\right)}\rangle= x_{1}^2 + x_{2}^2 + \dots + x_{N}^2
$$
...
$$ 
\langle\mathbf{x}_{\left(N \times 1\right)},\mathbf{x}_{\left(N \times 1\right)}\rangle = \sum_{n=1}^{N} x_{n}^2
$$
...
The inner product of a vector with $\left(N \times 1 \right)$ dimension with itself is a scalar, $\left(1 \times 1 \right)$. The outter product of a vector with $\left(N \times 1 \right)$ dimension with itself is a matrix, $\left(N \times N \right)$
