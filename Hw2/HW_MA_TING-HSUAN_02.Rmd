...
$$ 
log[p(H|AB,\mu)] \propto Hlog[\mu] + (AB-H)log[1-\mu] 
$$
...
tibble::tibble(
  mu = seq(from = 0.1, to = 0.6, by = (0.6-0.1)/(101-1))
) %>% 
  mutate(log_lik = player_hits*log(mu) + (player_atbats-player_hits)*log(1.0-mu) 
         ) %>% ggplot(mapping = aes(x=mu, y=log_lik)) +
  geom_line() +
  geom_vline(xintercept = player_mle, color = 'navyblue')
```
The MLE is corresponding to the mode of the distribution.
...
{
  # assign log_lik
  log_lik = events*log(prob) + (trials-events)*log(1.0-prob)
...
{
  log_coef = lchoose(trials, events)
  log_lik = log_binom_pmf_unnorm(events, trials, prob)
  return(log_coef + log_lik)
}
```
...
tibble::tibble(
  ...
  geom_vline(xintercept = player_mle, color = 'navyblue')
```
Yes, the MLE still corresponded to the same location as the un-normalized log-likelihood. Since 'log_binom_pmf' multiplies the log-likelihood by a constant, normalizing it, therefore the shape of the distribution does not change.
...
coin_probs <- dbinom(c(0:5),11,0.25)
# print to screen
coin_probs
# length?
length(coin_probs)
...
The first element of 'coin_probs' corresponds to the probability to of getting all tails, $(1-0.25)^{11} = 0.04223514$.
...
The results are exactly the same $0.9656725 == 0.9656725$
...
```{r, solution_02g}
pbinom(c(175:195),602,0.3)
pbinom(c(165:205),602,0.3)
```
...
$$
Posterior \propto Likelihood \times Porior
$$
where likelihood have the form of,

$$
Likelihood \propto Binomial(m|\mu,N) = \mu^{m}(1-\mu)^{N-m}
$$
and prior have the form of,

$$
Prior \propto Beta(\mu|a,b) = \mu^{a-1}(1-\mu)^{b-1}
$$
Taking the log of the un-normalized posterior and plugging in the un-normalized log-likelihood and un-normalized log-prior,

$$
log(Posterior) \propto mlog(\mu) + (N-m)log(1-\mu) + (a-1)log(\mu) + (b-1)log(\mu-1)
$$
Comebining like terms,

$$
log(Posterior) \propto (m+a-1)log(\mu) + (N-m+b-1)log(\mu-1)
$$
It is seens that the log posterior have the same form as the beta distribution, therefore the above equation can be rewritten as,

$$
log(Posterior) \propto Beta(\mu|a+m,b+(N-m))
$$
where the posterior Beta hyperparameters $a_{new} = a+m$, and $b_{new} = b+(N-m)$.
...
Since the Beta distribution hyperparameter $a$ represents the number of events, "hits", and $b$ represents the number of non-evetns, "misses", the number of "at bats" is the sum of the "hits" and "misses".
...
```{r, solution_03d}
qbeta(p = 0.05, a_prior, b_prior)
qbeta(p = 0.95, a_prior, b_prior)
```

The MLE for the first 30 bats is 0.2, which is close to the 5th percentile is 0.2199853. The MLE for the season is 0.3139535, which is close to the 95th percentile, 0.3398887 beta distribution.
...
The posterior mean of the first 30 bats is 0.2652 compared to the MLE of the first 30 bats is 0.2. These two values are close, but the posterior mean is a larger value. The posterioir mean of all the 602 bat is 0.3068, while the MLE for 602 bats is 0.31 which is very close.
...
```{r, solution_03g}
qbeta(p = 0.05, a_post_start, b_post_start)
qbeta(p = 0.95, a_post_start, b_post_start)

qbeta(p = 0.05, a_post_player, b_post_player)
qbeta(p = 0.95, a_post_player, b_post_player)
```

The MLE for the first 30 bats is not contained within the 90% uncertainty intervals, but the MLE for 602 bats is contain between the 90% uncertainty intervals.
...
Since 'pbeta()' returns the probabilty of the Beta random variable less than a particular quantity, the probability of a player's hits greater than 0.3 is,

```{r, solution_03h}
1 - pbeta(0.3, a_post_player, b_post_player)
```

Based on the probability returned above, I would say that this player is a good hitter for the complete season. The player's hit probabilty greater than 30% is 65%, meaning the player will hit the ball $0.7 \times 0.65=0.445$, 44.5% of the time. This value is high when compared to the $\mu_{ML} = 0.31$ calculated in problem 1.
...
The general formulation for log-posterior,
$$
...
$$
log \bigg[\prod_{n=1}^{N} \Big(\mathrm{normal}( x_n \mid \mu, \sigma)\Big) \bigg] = log \bigg[ \prod_{n=1}^{N}\frac{1}{\sigma \sqrt{2\pi}} Exp \Big( \frac{-1}{2\sigma^2}(x-\mu)^2 \Big) \bigg]
$$
dropping the constants, the un-normalized log-likelihood is,

$$
log \bigg[\prod_{n=1}^{N} \Big(\mathrm{normal}( x_n \mid \mu, \sigma)\Big) \bigg] \propto \sum_{n=1}^{N} \bigg\{ -\frac{(x-\mu)^2}{2\sigma^2} \bigg\}
$$
Note that a constant will come out from the log-likelihood, but neglecting it will return us the form necessary for the un-normalized log-posterior. The log-prior is,

$$
log \Big[\mathrm{Exp}(\sigma \mid \lambda) \Big] = log \Big[ \lambda \exp (-\lambda \sigma ) \Big]
$$
Ignoring the constants, the un-normalized log-prior is,

$$
log \Big[\mathrm{Exp}(\sigma \mid \lambda) \Big] \propto -\lambda\sigma
$$

Putting the un-normalized log-likelihood and log-prior together, the un-normalized log-prior is proportional to,

$$
log \Big[p(\sigma|x,\mu) \Big] \propto \sum_{n=1}^{N} \bigg\{ -\frac{(x-\mu)^2}{2\sigma^2} \bigg\} -\lambda\sigma
$$
...
$$
log \Big[p(\sigma|x,\mu) \Big] \propto \frac{-vN}{2\sigma^2} -\lambda\sigma
$$
Quantity v is referred to as being "sufficient" because it represnets the mean of the problem. After using $v$, the log-posterior does not depend on the individual observations, just the number of trails is needed.
...
{
  log_post = -0.5*v*N/sigma^2 - lambda*sigma
  return(log_post)
}
```
...
tibble::tibble(
  sigma = seq(from = 0.5, to = 4.5, by = (4.5-0.5)/(101-1))
  ) %>% 
...
The shape of un-normalized log-posterior vs. $\sigma$ is not a parabola. However, it is an exponential graph showing that towards higher $\sigma$ values, the log-posterior converges. This graph should only have one mode because we provided it with a labmda of 0.75.
...
The inverse of thte log is the exponential, therefore sigma can be represented as,
$$
\sigma = g^{-1}(\varphi) = Exp(\varphi)
$$
Takin the partial derivative of the inverse link function with respect to $\varphi$,
$$
\frac{\partial}{\partial \varphi}g^{-1}(\varphi) = \varphi Exp(\varphi)
$$
Taking the log of the partial derivative of the inverse link function with respect to $\varphi$,
$$
log \Big(\frac{\partial}{\partial \varphi}g^{-1}(\varphi) \Big) = log(\varphi) + \varphi
$$
Therefor the un-normalized log-prior of the log-derivative adjustment is,
$$
log \Big(\frac{\partial}{\partial \varphi}g^{-1}(\varphi) \Big) = \varphi
$$
### 4f)
...
log_post_varphi_unnorm <- function(varphi,N,v,lambda)
{
  ...
  log_post <- -0.5*v*N/sigma^2
  
  # account for the derivative adjustment
  log_adjust = log_post + varphi
  return(log_adjust)
}
```
...
tibble::tibble(
  varphi = log(seq(from=0.5,to=4.5,by=(4.5-0.5)/(101-1)))
  ) %>% 
  ...
  geom_line()
```
The log-posterior looks for parabolic compared to the previous graph without utilizing the link function.