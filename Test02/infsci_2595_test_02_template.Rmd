...
### Solution 2e)
Each observation will be used 5 times per model if we were to use a 5-fold corss-validation.
...
### Solution 2f)
Using 5-fold cross-validation, we would train the model 5 times, and testing each training model 5 times for a total of 25 validations.
...
### Solution 3h)
A threshold value of 0.5 is used to construct the ROC curve.
...
### Solution 3k)
A possible accuracy values by using a LOO cross-validation is 0.9. ?? I dont know...
...
$$
$$
\frac{\delta y_n\log(\mu_{n}) - \log(y_n!) - \mu_n}{\delta \mu_n} = \frac{y_n}{\mu_n} -1
$$
$$
