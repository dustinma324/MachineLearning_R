...
prob_01_df %>% ggplot(aes(x = x1)) +
  geom_bar(aes(fill = x2), position = "dodge")
```
There is not a specific combination of `x1` and `x2` that dominate the count in each data set. It is fairly uniform between the two discrete varavbles.
...
lf_01 %>% ggplot(aes(x1,input_value)) +
  geom_boxplot(aes(fill = x2)) +
  facet_wrap("input_name")
```
No, the continuous variables from `x3` to `x6` all look relatively similar with regards to the discrete inputs.
...
lf_01 %>% ggplot(aes(input_value,y)) +
  geom_point(aes(x2)) +
  facet_grid("x1")
```
It seem like when `x2` is in the "a" category, the responses doesn't seem to vary even when `x1` changes. This behavior cannot be said about when `x2` is in category "b", as varition in the continuous response changes across different categories of `x1`.
...
```
No, the continuous inputs are not correlated at all.
...
set.seed(3321)
fit_01_lm <- train(y ~ x1 + x2 + x3 + x4 + x5 + x6,
                   data = prob_01_df,
                   method = "lm",
                   metric = metric_01,
...
fit_01_lm
```
About 280 samples are used to test the model each fold. The cross-validation averaged R-squared value is 0.439, which is ok.
...
```
Yes, three out of the eight coefficients have a confidence interval that contains zero. The meaning of a confidence interval containing zero is that there might be a chance where those specific coefficients have no effect on the model.
...
Because the discrete input of `x1` is countned as well. The cross-validation fitting have determined that when `x2` is in category "b", and `x1` in bategory "B" and "C" will have the best performance.
...
```{r, solution_02e_a}
model.matrix(y ~ (.)^2, prob_01_df) %>% ncol()
...
fit_01_lm_pairs
```
Yes, using pair-wise interaction provided a similiar R-squared value compared to that of the 5 fold cross-validation.
...
```
The contribution from continuous input `x3`, discontinuous input `x2b`, pair-wise `x1C:x2b`, and pair-wise `x1B:x3` is signinifcant to the fitting of this model.
...
fit_01_glmnet_trips
```
Based on the result from the decault grid, the elastic net model is more on the Lasso regression side.
...
fit_01_nnet
```
As the size increase, the Rsquared and RMSE value starts to get better and better compared to itself and the `lm` model.
...
fit_01_rf
```
mtry = 7 was considered the best, and it corresponds to the number of variable randomly selected per split.
...
plot(fit_01_xgb)
```
150 tress showed the best result. 
...
```
The boosted tree method is considered the best.
...
```
The most inportant value is discrete input variable `x2b`, and the least important is continuous input variable `x6`.
...
Yes, because you have some inputs in the middle that have confidence interval of greater than 1.  
...
Sonar %>% 
  dplyr::select(-Class) %>%
  cor() %>% corrplot::corrplot(method = "square", order = "hclust")
```
There isn't a huge correlation between the inputs, but towards the begining of the input there is some correlation and some anticorrelation.
...
```{r, solution_5b, warning=FALSE, eval=TRUE}
set.seed(4321)
...
fit_glm_sonar
```
The arear under the curve is 0.769.
...
fit_glmnet_sonar
```
Based on the training results, the elastic net significantly favors ridge regression.
...
fit_rf_sonar
```
mtry = 2 was selected as the best, it was picked because $mtry = \sqrt(D)$, and D corresponds to classes, and is 2 in our case.
...
```
Random forest is shown as the best method for this data set.
...
```
6 variables are above 70% importance.
...
test_sonar_grid %>% 
  ...
  theme_bw()
```
The elastic net shows almost a linear relationship between variable `V11` and `v12` with the prediction of the outcome. As both variable increases, the more metal classification appears. This behavior is also shown in the random forest classification, where `V11` and `V12` increases, the number of prediction for metal detection increases. 
...
test_sonar_grid %>% 
  ...
  theme_bw()
```
The predicted probabilities for the "M" level are consistent with the classification surfaces visualized in Problem 6c) as they look the same, and follows the general linear profile for elastic net, and nonlinear profile for random forest.
